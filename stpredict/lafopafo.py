...
def lafopafo(data: list,
    ...
    if data[0]['spatial id'].nunique() == 1:
        ...
        if len(performance_measures) == 0:
            sys.exit("Error: The input 'performance_measures' cannot be calculated for the single test instances of the last fold partitioning forecaster.")
        if 'AUC' in performance_benchmark:
            sys.exit("Error: The input 'performance_benchmark' cannot be calculated for the single test instances of the last fold partitioning forecaster.")
        if 'R2_score' in performance_measures:
            sys.exit("Error: The input 'performance_benchmark' cannot be calculated for the single test instances of the last fold partitioning forecaster.")
        if 'AUPR' in performance_measures:
            sys.exit("Error: The input 'performance_benchmark' cannot be calculated for the single test instances of the last fold partitioning forecaster.")
